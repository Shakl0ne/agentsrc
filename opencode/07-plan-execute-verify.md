...
插件注册了十几个 agent，但论证主线只需关注三个：规划师（plan）、指挥者 + 执行者（execute）、验证关卡（verify）。名册里其余的角色——如顾问、评审、检索、视觉审查等——服务于更精细的分工，但对"范围漂移 / 验证缺口"这条主线不构成新的设计论点。下文只在这三个核心角色上展开——选择性地砍名册、留机制，因为这一章的要点是"编排的骨架"而不是"插件有多少种开会方式"。

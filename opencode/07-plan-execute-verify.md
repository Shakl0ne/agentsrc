...
让 agent 又动手又验证、还要防着它跑偏，光靠一条 runLoop 单循环够吗？本系列前面把内核的 runLoop、工具、Agent、上下文都拆过了，这一篇换一个载体回答这个问题：一个构建在 OpenCode 之上的开源插件 `oh-my-openagent`（4.9.2），把"一个单体 agent"重构成"一支分工明确、有状态、有验证关卡的小团队"。它没给内核加魔法，只是**用原生的积木搭出 plan / execute / verify 三种有界角色**，每个角色配一道硬约束锁住它能碰的操作范围。这篇就把这套编排怎么运转拆开看：

...
一个单循环 agent 在真实工程里常会撞出这样一幕：它一边读代码定位问题、一边动手改、一边又回头查文档，读与写的角色在同一上下文空间内高度耦合，改着改着就偏离了最初目标；最后它在答复里写下"完成了、测过了"，却没有任何机制能证明这句话。这是很多 agent 系统都逃不掉的毛病。
...
把论证链收口：**范围蔓延**由 Plan 的只读硬边界与 Execute 的 checkbox 回写双重承接（决策权在规划态被隔离，改动范围在执行态被钉死）；**验证缺口**由 Verify 的对抗性关卡堵住（完成必须有证据、且最终要过用户审批）。三层硬约束，两个失败模式，设计闭环。

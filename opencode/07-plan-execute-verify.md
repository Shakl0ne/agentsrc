...
本系列前面几篇把 OpenCode 内核的 runLoop、工具系统、Agent 系统、上下文架构都拆过了。这一篇换一个载体：一个构建在 OpenCode 之上的开源插件 `oh-my-openagent`（4.9.2），它把"一个单体 agent"重构成"一支分工明确、有状态、有验证关卡的小团队"，用的是原生的积木、没有给内核加魔法。下文先看单循环为什么堵不住这两个毛病，再看它如何拆成 plan / execute / verify 三态、每态各自被一道什么样的硬约束锁住，最后落回这套编排对原生内核的启示。

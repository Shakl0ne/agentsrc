...
一个单循环 agent 在真实工程里常会撞出这样一幕：它一边读代码定位问题、一边动手改、一边又回头查文档，读和写的角色混在同一条上下文里，改着改着就偏离了最初目标；最后它在答复里写下"完成了、测过了"，却没有任何机制能证明这句话。这是很多 agent 系统都逃不掉的毛病。

让 agent 又动手又验证、还要防着它跑偏，光靠一条 runLoop 单循环够吗？本系列前面把内核的 runLoop、工具、Agent、上下文都拆过了，这一篇换一个载体回答这个问题：一个构建在 OpenCode 之上的开源插件 `oh-my-openagent`（4.9.2），把"一个单体 agent"重构成"一支分工明确、有状态、有验证关卡的小团队"。它没给内核加魔法，只是**用原生的积木搭出 plan / execute / verify 三种有界角色**，每个角色配一道硬约束锁住它能碰的操作范围。

本文将深入拆解这套编排机制的运转逻辑：

- **第一**，为什么要把一条循环拆成三个角色、各自隔离权力边界；
- **第二**，plan / execute / verify 各拿哪一道硬约束，守住"该做的"和"不该碰的"；
- **第三**，这套编排最终给原生内核带来什么启示。

载体是你自己也能装的 `oh-my-openagent` 插件，不是内核新增的神秘能力——每一层都用原生钩子和工具白名单搭得出来。
...
### 1.1 范围蔓延：边读边写、角色混乱、越改越偏

想象你让一个单循环 agent 去"重构 auth 模块，顺便加个限流"。它通常的轨迹是：读二十来个相关文件 → 在上下文里逐步构建出对代码库的理解 → 直接开始 `Edit` 十几个文件 → 中途发现某个依赖要查，再回头看代码 → 改着改着，最初那条"不要动数据库 schema"的约束从上下文里被挤掉了。这不是模型笨，而是它把"设计决策"和"逐文件修改"塞进了同一个上下文，每读一个文件产出的新理解都在悄悄改写它自己对目标范围的心智模型。

范围蔓延的本质是**决策与执行不分开**。一个同时握有"决定改哪里"和"实际动手改"两种权力的 agent，会在执行过程中不断重新解释自己手里的范围——因为它每读一个文件，目标就可能被微调一次。这也让任务的可复现性变差：同样的需求，上一轮和下一轮可能收敛到完全不同的改动面。
...
runLoop（见本系列第二章）的设计目标是"不间断地把一轮工具调用跑完"，它没有区分规划态、执行态、验证态。它把三种不同的脑力负担——想清楚要做什么、照着做出来、证明做的对——揉进同一条循环，于是两个失败模式同时得手：范围蔓延来自"决策权与执行权不分"，验证缺口来自"自证代替了关卡"。
...
![三态分工与硬约束模型](/images/opencode/07-plan-execute-verify-roles.svg)
...
插件注册了十几个 agent，但论证主线只需关注三个：规划师（plan）、指挥者 + 执行者（execute）、验证关卡（verify）。名册里其余的角色——如顾问、评审、检索、视觉审查等——服务于更精细的分工，但对"范围蔓延 / 验证缺口"这条主线不构成新的设计论点。下文只在这三个核心角色上展开——选择性地砍名册、留机制，因为这一章的要点是"编排的骨架"而不是"插件有多少种开会方式"。

## 三、Plan：只读规划是怎么被"硬强制"实施的

规划态是整套编排的起点，也是范围蔓延的防线。它的设计要点在于：**规划权必须从执行权里剥出来**，并且这个剥离不能只靠提示词，要用硬边界锁死。
...
规划唯一的落盘产出是 `.omo/plans/*.md`。这份文件不仅是给人看的目标清单，更是整条编排的**契约**：执行端照着它打勾，验证端靠它判断进度。计划里每一项任务都是一个 checkbox（`- [ ] N. 任务`），执行时把它勾成 `- [x]`。计划文件与执行进度的绑定，让"要做什么"从模型的内存落到了磁盘上可审计的文件里——这正是范围蔓延的对立面：范围被钉在文件里，而不是飘在上下文里。
...
![计划与状态机的双重闭环](/images/opencode/07-execution-state-machine.svg)
...
![对抗性验证的三重关卡](/images/opencode/07-verification-gates.svg)

执行阶段只能保证"按计划推进"，却无法确保"结果完全正确"。这正是验证机制（Verify Phase）发挥作用的场景。与多数系统采用的"先信任后验证"不同，OpenCode 的设计原则更为审慎——它默认子 Agent 的输出不可信，从而将"结果验证"从可选动作提升为不可绕过的硬性关卡。
...
这一段的含义是：**成功的唯一证据，是一个磁盘文件里的 checkbox 从"未勾"变成"已勾"**。完成门并不接受"我把代码改了"这类叙述，它强制执行者在宣称完成之前，用一次写 + 一次读把可核对的进度变化落进计划文件。没走完这道门，执行者的"完成"不会被记录——它直接堵住了"模型说做完了"这种验证缺口的最浅形态。
...
- **关卡三：Final Verification Wave**——当所有实现 todo 都进入"已勾"，开始收尾的统验批次，它是**把终审结论集中交给用户的唯一审批时刻**：计划里的 `F1-F4` 是终审项，由 `shouldPauseForApproval` 决定是走 final-wave 审批还是普通完成门：
...
| 阶段 | 硬约束 | 堵住哪个失败模式 |
|------|--------|-----------------|
| Plan | 只读硬边界（allowed-tools + hook reject） | 范围蔓延（决策权不越界） |
| Execute | 状态机（boulder + checkbox 回写） | 范围蔓延（不偏离计划）+ 可恢复 |
| Verify | 验证关卡（Gate + 独立 Review + Final Wave） | 验证缺口（证明替代自述） |

这三条约束是正交的：只读边界管"不能越界"、状态机管"不偏离且能续"、验证关卡管"完成可证明"。任何一个失败模式都至少被其中一层承接。这正是第一节那段推理的最短落实——单一循环堵不住的两个问题，被三态各自的硬约束分段堵住。

### 6.3 回扣主线 + 总对比

把论证链收口：**范围蔓延**由 Plan 的只读硬边界与 Execute 的 checkbox 回写双重兜底（决策权在规划态被隔离，改动范围在执行态被钉死）；**验证缺口**由 Verify 的对抗性关卡堵住（完成必须有证据、且最终要过用户审批）。三层硬约束，两个失败模式，设计闭环。
...
| 保障机制 | 提供工具与钩子，不决定策略 | 在原生钩子上把"严格角色分离 + 可证明完成"编排成纪律 |

一句话回扣主线：**这套编排的价值在于用最少的抽象层把"想、做、验"三个本来挤在单循环里的负担拆开，并用只读边界、状态机、验证关卡三道硬约束各自堵住一个失败模式**。它对原生内核的启示是：能抗住范围蔓延与验证缺口的，不是更强的模型，而是"决策权、改动权、评判权互相隔离 + 每权都有一道它破不了的硬边界"的编排纪律——所有积木，原生都已给齐。

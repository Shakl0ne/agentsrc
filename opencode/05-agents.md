...
- **第三**，父子 session 的边界靠什么锁住，为什么子 agent 只能活在父级收窄后的权限边界之内；
...
### 6.2 设计复盘：子 agent 的适用边界与工程启示

- **该用**：
  - 上下文隔离：子任务拥有独立的上下文，避免与父会话互相污染，也无需继承庞大的历史对话。
  - 权限隔离：实现粒度更细的权限控制，例如把只读权限透传给它、确保探索或研究类子 agent 只查不写。
  - 模型分工：针对特定任务路由到专用模型，例如把轻量查询交给低成本模型、把复杂推理沉到高算力模型。

- **别用**：
  - 任务过于轻量：创建子会话、装配 Prompt 以及解析响应契约的工程开销，已超过任务本身的执行成本。
  - 重度依赖主会话上下文：若必须向子 agent 复制大量父会话历史，这种上下文搬运的成本将远高于在原会话中直接执行。

落到工程启示：多 Agent 系统的设计核心在于"掌控治理开销"。OpenCode 没有盲目地把每个微任务都拆成独立 agent，而是把"委派"收敛成一条可 Schema 化、可隔离、可回收的标准管道，要不要执行由主 LLM 根据任务自行判断。

在架构实现上，OpenCode 展现了清晰的取舍：调度层保持极简（单队列 `pop()` 串行），隔离层维持刚性（权限派生与防递归），回收层注重稳健（结构化 XML 契约）。面对并发需求，它既可以用 `background` 支持后台异步，也能把高阶协同交由上层 coordinator 编排。对于绝大多数"委派即独立子任务"的场景，OpenCode 用最简洁的架构给出了最稳的解法——工程设计的价值在于精准把控边界与复杂度，而非把每一项能力都堆砌到极限。

...
写多 Agent 系统时，最常面对的问题是"委派"：主 agent 手里来了一个太大、太杂、最好跟当前上下文隔开的任务，要不要把它交给另一个执行者。让主 agent 在现有上下文里硬做，几万 token 的历史会拖慢它、干扰它；拆给一个子 agent，又得先回答四个问题——怎么描述执行者、怎么发起委派、怎么把它和父隔离、结果怎么安全收回。

OpenCode 对这套问题的答案一以贯之：把"委派一个子 agent"实现成一段 **schema 化 + 占位式分发 + 权限派生** 的抽象链，而不是一段套娃的调用。子 agent 的执行被锁在"父内缩权限"的领地里，结果以可解析的 XML 收回来。下文沿着"描述执行者 → 委派入口 → 边界隔离 → 执行与回收 → 调度取舍"五段拆下去，最后做一个横向收束。源码里那些 mode 枚举、task 工具、`deriveSubagentSessionPermission`、`tasks.pop()`，是一层一层的设计证据。

## 一、委派要先能"描述执行者"：agent 的声明式 schema

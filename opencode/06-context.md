...
让 Agent 理解项目，最朴素的想法是给它开一块"记忆"：把代码风格、用户偏好、往期对话都记下去，下次直接查。可现实很快撞到一句绕不开的话——项目风格写在文件系统里、聊过的内容存在数据库里，而一次调用模型时，能塞进窗口的 token 是有限、并且要花钱的。

一个 Agent 引擎要怎么"既记住项目、又不把每次调用的窗口撑爆"？OpenCode 给出的答案不是造一个抽象的记忆层，而是把"记住"拆成一条**装配链**：谁该常驻磁盘、谁按需进入窗口、谁每次请求级拼装，最后又由哪一道预算管决定装不下的怎么办。本文先搭起这条链的总地图。

这篇能回答三个问题：

- **第一**，知识该被分成哪几类、凭什么一类常驻磁盘、一类请求级注入；
- **第二**，读文件时上下文是怎么按需"长"进窗口的；
- **第三**，窗口装不下时谁负责截、跨会话的信息又靠什么活过下一次打开。

压缩机制的 prune / compact 细节本系列第四章已单独拆过，这里只顺着链条讲它们的位置和作用，不重复展开。

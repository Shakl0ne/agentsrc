...
将"判定 → 触发 → 骨架 → 分层 → 续接"收拢为五条清晰的逻辑线：

**判定机制：统一的公共开关**

溢出判定是系统的公共基座。`usable()` 预留输出空间，`isOverflow()` 则对比当前占用与可用额度；一旦 `auto` 开关关闭或 `limit` 为零即触发短路。这套统一逻辑，构成了主动（proactive）与被动（reactive）触发的公共前提。

**触发路径：双轨并行，共用创建**

触发路径分为两条：Proactive（主动式）在 Assistant 跑完后主动检查，Reactive（被动式）则捕获 `ContextOverflowError` 异常进行兜底。二者均调用 `create` 建立任务，核心差异仅在于是否带有 `overflow` 标记。

**执行骨架：占位派发，解耦解包**

骨架设计采用"占位 + 异步派发"模式：`create` 仅插入占位 `user` 消息；`runLoop` 负责轮询分发；`process` 真正调用 LLM 生成；而 `prune`（精简）则在收尾处异步拉起，完全不阻塞主流程响应。

**分层保留与防嵌套摘要**

保留方面，Prune 默认保护最近 2 轮对话、40K token 预算及 Skill 内容；`select` 同样默认保留 2 轮，将预算严格钳制（clamp）在 8K，超限时通过 `splitTurn` 寻找精准切点。摘要则由专用 Agent 执行，禁用工具以防止递归嵌套；通过锚定历史实现增量更新，即便 9 段式模板中存在空段落也予以保留。

**无缝续接与数据可逆性**

续接方面，为了保证 LLM 上下文连贯（无断裂感），`filterCompacted` 会将未压缩的尾部消息平移至摘要之后；而 `overflow` 处理则优先重放用户的原始提问，若无原提问则合成 `"continue"` 指令引导模型续写。数据可逆方面，Prune 仅通过时间戳标记数据而非物理删除，数据库层依然可追溯；只有进入 Compact 阶段才会对实际上下文进行改写，实现了"可逆性"与"两段分离"。

回看整条链路，OpenCode 这套两级压缩机制的核心价值在于"精准取舍与数据可逆"，而非单纯追求"压缩量最大化"。它没有把精力耗费在中心化维护复杂的缓存感知多级阶梯上，而是把"删哪些、怎么续"这两个关键决策做深做透，并用数据可逆性兜底工程风险。这种在复杂度与系统成本之间取得的动态平衡，比盲目将上下文压到极限更值得 Agent 系统借鉴。

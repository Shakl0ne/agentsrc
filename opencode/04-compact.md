...
续接方面，为了保证 LLM 上下文连贯，`filterCompacted` 会将未压缩的尾部消息平移至摘要之后；而 `overflow` 处理则优先重放用户的原始提问，若无原提问则合成 `"continue"` 指令引导模型续写。数据可逆方面，Prune 仅通过时间戳标记数据而非物理删除，数据库层依然可追溯；只有进入 Compact 阶段才会对实际上下文进行改写，实现了"可逆性"与"两段分离"。

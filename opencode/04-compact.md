...
OpenCode 没把压缩做成一个“快满了再临时补锅”的小功能，而是把它组织成一条完整链路：先判断窗口有没有逼近上限，再决定走哪种回收方式，压完之后还要把会话稳稳续上。一端是不调 LLM 的 Prune，只回收旧的工具输出；另一端是调 LLM 生成锚定摘要的 Compact，回收的是对话历史。两条路径共用同一套溢出判定和续接管道。
...
- **第五**，OpenCode 和 Claude Code 的差别，不在“会不会压”，而在压缩层级和成本取舍。

...
const q = [
  ...
  },
  {
    ...
    options: ['因为工具调用都由数据库触发，无法在内存里循环', '因为工具调用只是让主循环的退出条件暂时不成立', '因为 AI SDK 不支持连续两轮之间保留工具上下文', '因为工具执行和模型调用必须分属两条不同的 session 控制流'],
    correct: 1,
    explanation: 'OpenCode 没把工具调用拆成独立循环，而是统一交给主循环的“继续 / 停下”判断来推进。只要模型发出了工具请求，或消息流里还有未处理的 tool part，session 就还没到可退出状态，主循环自然继续下一轮。问题不在数据库、AI SDK 或 session 控制流拆分。'
  },
  {

---
title: OpenCode 主循环：一条消息是怎么跑完整个 Agent 的？
---

# OpenCode 主循环：一条消息是怎么跑完整个 Agent 的？

你在终端里按下一次回车，看上去只是“发给 Agent 一条消息”。可系统内部紧接着要把输入写进会话、拉起模型、执行工具、回流结果、处理中断，还得保证同一个 session 不会被两条执行链同时写坏。只要其中一环没收住，工具调用、压缩、重试、并发就会互相打架。

那一条消息到底是怎么被稳稳推进到底的？OpenCode 把答案收敛在一条主循环里，但关键不在 `while (true)` 这几个字，而在它如何围绕 **session 状态**持续推进：什么时候继续、什么时候停下、工具结果怎么回流、窗口装不下时又怎么切到压缩路径。先看下面四个要点：

- **第一**，为什么说 OpenCode 的主循环本质上是一台会话状态机；
- **第二**，工具调用为什么不是外挂的小循环，而是靠“继续 / 停下”的条件一路往前推；
- **第三**，compaction 为什么会成为主循环里的常规路径，而不是临时补丁；
- **第四**，`prompt.ts`、`processor.ts`、`llm.ts` 这几层边界是怎么把主循环撑稳的。

这一篇只拆主循环本身的推进逻辑；工具系统、上下文压缩和多 Agent 委派的细部机制，会在后面的章节各自展开。
...
- **防并发跑飞**：同一个 session 里，不能因为用户连按两次回车，就起两条互相竞争同一份状态的执行链
- **让模型和工具轮流推进**：模型先说话、工具再执行、结果回流给模型，直到任务真的结束
- **在风险出现时自救**：上下文超长、中断、重试、工具死循环，这些都不能把会话搞坏

如果把这四件事拆成四套互不相干的流程，系统很快就会变成一团浆糊。一个工具执行到一半触发压缩；压缩刚做完，子任务又要插队；用户这时再发一条新消息——执行立刻乱套。

所以主循环要处理的，是**怎么让所有待处理工作回到同一个决策口。**

### 1.2 OpenCode 的答案：一台状态机

OpenCode 的处理方式很克制。它没有把所有逻辑挤在一个巨大的入口函数里，而是将处理流程拆为了三层：
...
OpenCode 在这里把两块边界拆开：编排逻辑交给 `runLoop()`，并发控制单独抽成门闸。不要把两件事混在同一层。也正因为这样，输入、并发、编排三种复杂度才没有被塞进同一个入口函数里。

![Three Entry Boundaries](/images/opencode/02-layers.svg)
...
OpenCode 走的是另一条路：**所有需要继续推进的情况，都回到同一个 `while (true)` 顶部重新判断，让"是否继续"的决策收敛到同一处。**
...
主循环之所以能只专注“控制走向”，是因为繁重的杂活都被下沉到了两个边界层：
...
![One Loop, Many Work Types](/images/opencode/02-loop.svg)
...
放在这条链路里看，`processor` 就是事件流和会话状态之间的承接层。
...
OpenCode 这套写法没有追求花哨，它关心的是怎么把主循环失控的风险压住。
...
const q = [
  ...
  {
    question: 'OpenCode 为什么不把工具调用单独写成一层 `while` 循环？',
    options: ['因为工具调用都由数据库触发，无法在内存里循环', '因为工具调用的作用是让主循环的退出条件暂不成立', '因为 AI SDK 不支持连续两轮之间保留工具上下文', '因为工具执行和模型调用必须分属两条不同的 session 控制流'],
    correct: 1,
    explanation: 'OpenCode 没把工具调用拆成独立循环——一旦拆开，工具循环和主循环就成了两套互相通信的状态机。它选择让工具调用只是让主循环的退出条件暂不成立：只要模型发出工具请求、或消息流里还有未处理的 tool part，session 就没到可退出状态，主循环自然进入下一轮。不拆并不是做不到，而是为了避免多加一套平行循环；这跟数据库、AI SDK 或 session 控制流拆分都没有关系。'
  },
  {

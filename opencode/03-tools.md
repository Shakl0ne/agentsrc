...
写自己的 AI Agent，最先塌房的往往不是模型本身，而是工具系统。让 Agent 调 Shell 怕它跑危险命令，让它写文件怕它越界删库，每次调外部 API 都弹窗批准会把用户烦死，而 LLM 填参数还动不动就跑偏。要是把这些安全防线分散到每个工具里各写一遍，项目很快就会沦为一堆打补丁硬凑出来的屎山。
...
把这两道防线统一收拢到包装层（Wrap），意味着后续无论是新增校验规则还是调整截断策略，都只需修改一处即可全局生效——这就是将横切关注点统一收拢的核心收益。关于具体的截断判定与落盘细节，请参考下方图解：

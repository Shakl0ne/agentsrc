...
const q = [
  {
    ...
    options: [
      'JavaScript 引擎普遍不支持尾调用优化',
      '流式 API 特性匹配七种异常处理需求',
      '在循环中方便使用 async generator 的 yield',
      '整体循环代码行数得以压缩到最低'
    ],
    correct: 1,
  ...
  },
  {
    ...
    options: [
      '通过回调函数实现双向消息传递与错误传播',
      '基于事件发射器达到模块间松散耦合效果',
      'async generator 产出消息供外层 for await 消费',
      '借助共享内存变量以减少模块间通信开销'
    ],
    correct: 2,
  ...
  },
  {
    ...
    options: [
      '满足 TypeScript 类型系统对标志位的静态约束',
      '防止压缩失败后陷入无限循环耗尽 API 调用预算',
      '不同压缩路径产出的消息格式差异需要标志位区分',
      '用于向遥测系统上报压缩路径的执行统计信息'
    ],
    correct: 1,
  ...
  },
  {
    ...
    options: [
      '不设上限属于设计失误后续通过补丁修复了该问题',
      '由调用方传 maxTurns 配合成本上限与拒绝追踪兜底',
      'Claude 模型自身的停止机制会确保不无限循环下去',
      'OpenCode 与 Codex 同样不设默认的上限值'
    ],
    correct: 1,
